...
struct attr_generator_offset<num_attrs> {
  constexpr static std::array<unsigned long, num_attrs> gen(unsigned long, unsigned int) {
...
template <typename Data_t, unsigned int M, unsigned int N, unsigned int PackSize, typename... Ts>
...
constexpr bool can_apply(AttribHolder<FirstAttrib, SecondAttrib, Attribs...>,
  ...
  return data_attrib_pair<FirstAttrib, FirstParam>::exists &&
         can_apply(AttribHolder<SecondAttrib, Attribs...>(), ParamHolder<SecondParam, Params...>());
}
...
struct data_inserter<vec2> {
  static unsigned int insert(const vec2& data, char* buffer, unsigned int offset) {
...
struct data_inserter<vec3> {
  static unsigned int insert(const vec3& data, char* buffer, unsigned int offset) {
    float* temp_buf = (float*)(&buffer[offset]);
    temp_buf[0]     = data.x;
    temp_buf[1]     = data.y;
    temp_buf[2]     = data.z;
...
struct data_inserter<vec4> {
  static unsigned int insert(const vec4& data, char* buffer, unsigned int offset) {
    float* temp_buf = (float*)(&buffer[offset]);
    temp_buf[0]     = data.x;
    temp_buf[1]     = data.y;
    temp_buf[2]     = data.z;
