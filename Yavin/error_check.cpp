...
void gl_error_check(const std::string& function) {
  if
    constexpr(compilation == Compilation::Debug) {
      auto err    = glGetError();
      auto errStr = gl_error_to_string(err);
      if (err != GL_NO_ERROR) throw gl_error(function, errStr);
    }
}
...
void gl_framebuffer_not_complete_check(const GLuint fbo_id) {
  if
    constexpr(compilation == Compilation::Debug) {
      ...
      auto statusStr = gl_framebuffer_error_to_string(status);
      if (status != GL_FRAMEBUFFER_COMPLETE) throw gl_framebuffer_not_complete_error(statusStr);
    }
}
}  // Yavin
