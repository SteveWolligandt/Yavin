...
void BaseMesh<Ts...>::load_obj(
    ...
    std::function<void(size_t, const tinyobj::attrib_t&, const std::vector<tinyobj::shape_t>&,
                       const std::vector<tinyobj::material_t>&, const std::vector<size_t>&,
  ...
  if (attrib.normals.size() > 0) normal_indices.resize(attrib.vertices.size(), -1);
  if (attrib.texcoords.size() > 0) texcoord_indices.resize(attrib.vertices.size(), -1);
...
Mesh<vec3, vec3, vec2>::Mesh(const std::string& filepath) { load_obj(filepath); }

void Mesh<vec3>::load_obj(const std::string& filepath) {
  BaseMesh<vec3>::load_obj(
      filepath, [&](size_t i, const tinyobj::attrib_t& attrib, const std::vector<tinyobj::shape_t>&,
                    const std::vector<tinyobj::material_t>&, const std::vector<size_t>&,
                    const std::vector<size_t>&) {
        m_vbos.back().push_back(
...
void Mesh<vec3, vec2>::load_obj(const std::string& filepath) {
  BaseMesh<vec3, vec2>::load_obj(
      filepath, [&](size_t i, const tinyobj::attrib_t& attrib, const std::vector<tinyobj::shape_t>&,
        ...
        m_vbos.back().push_back(
            {attrib.vertices[i * 3], attrib.vertices[i * 3 + 1], attrib.vertices[i * 3 + 2]},
            ((attrib.texcoords.size() > 0) ? vec2{attrib.texcoords[texcoord_indices[i] * 2],
                                                  attrib.texcoords[texcoord_indices[i] * 2 + 1]}
                                           : vec2{0, 0}));
      });
}
...
void Mesh<vec3, vec3>::load_obj(const std::string& filepath) {
  BaseMesh<vec3, vec3>::load_obj(
      filepath, [&](size_t i, const tinyobj::attrib_t& attrib, const std::vector<tinyobj::shape_t>&,
                    ...
                    const std::vector<size_t>& normal_indices, const std::vector<size_t>&) {
        m_vbos.back().push_back(
            {attrib.vertices[i * 3], attrib.vertices[i * 3 + 1], attrib.vertices[i * 3 + 2]},
            ((attrib.normals.size() > 0) ? vec3{attrib.normals[normal_indices[i] * 3],
                                                attrib.normals[normal_indices[i] * 3 + 1],
                                                attrib.normals[normal_indices[i] * 3 + 2]}
                                         : vec3{0, 0, 0}));
      });
}
...
void Mesh<vec3, vec3, vec2>::load_obj(const std::string& filepath) {
  BaseMesh<vec3, vec3, vec2>::load_obj(
      filepath,
      [&](size_t i, const tinyobj::attrib_t& attrib, const std::vector<tinyobj::shape_t>&,
          const std::vector<tinyobj::material_t>&, const std::vector<size_t>& normal_indices,
          const std::vector<size_t>& texcoord_indices) {
        m_vbos.back().push_back(
            {attrib.vertices[i * 3], attrib.vertices[i * 3 + 1], attrib.vertices[i * 3 + 2]},
