...
class VertexBuffer : public Buffer<GL_ARRAY_BUFFER, data_representation<Ts...>> {
 public:
  class iterator;
...
template <typename... Ts>
VertexBuffer<Ts...>::VertexBuffer() : Buffer<GL_ARRAY_BUFFER, data_representation<Ts...>>() {}

template <typename... Ts>
VertexBuffer<Ts...>::VertexBuffer(const VertexBuffer& other)
    : Buffer<GL_ARRAY_BUFFER, data_representation<Ts...>>(other) {
  std::cout << "COPY CONSTRUCTOR OF VBO\n";
}

template <typename... Ts>
VertexBuffer<Ts...>::VertexBuffer(VertexBuffer&& other)
    : Buffer<GL_ARRAY_BUFFER, data_representation<Ts...>>(std::move(other)) {
  std::cout << "MOVE CONSTRUCTOR OF VBO\n";
}
...
void VertexBuffer<Ts...>::upload_data(bool keep_data_on_cpu) {
  ...
  if (!m_gpu_buffer_created) {
    ...
    auto gpu_buffer = reinterpret_cast<unsigned char*>(glMapNamedBuffer(this->m_id, GL_WRITE_ONLY));
...
constexpr void VertexBuffer<Ts...>::activate_attributes() {
  for (unsigned int i = 0; i < attr_prefs<Ts...>::num_attrs; i++) {
    ...
    glVertexAttribPointer(i, attr_prefs<Ts...>::num_dims[i], attr_prefs<Ts...>::types[i], GL_FALSE,
                          attrib_pack_size<Ts...>::value, (void*)attr_prefs<Ts...>::offsets[i]);
...
class VertexBuffer<Ts...>::iterator {
 public:
  iterator(VertexBuffer<Ts...>& vbo, const size_t idx) : m_vbo(vbo), m_idx(idx) {}
...
class VertexBuffer<Ts...>::const_iterator {
 public:
  const_iterator(const VertexBuffer<Ts...>& vbo, const size_t idx) : m_vbo(vbo), m_idx(idx) {}

  const_iterator(const const_iterator& other) : m_vbo(other.m_vbo), m_idx(other.m_idx) {}
 ...
 public:
  ...
  auto& operator[](const size_t idx) {
    return *reinterpret_cast<typename index_param_pack<N, Ts...>::type*>(
        &m_vbo(idx * Yavin::attrib_pack_size<Ts...>::value) + Yavin::attr_prefs<Ts...>::offsets[N]);
  }
