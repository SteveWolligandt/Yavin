...
struct RandomTexture1D : tex1D<type, C> {
  using parent_t = tex1D<type, C>;
  ...
  //----------------------------------------------------------------------------
  RandomTexture1D() : eng{std::random_device{}()}, data(w * num_components) {
    dist_t rand{min, max};
    auto   random_val = [&] { return rand(eng); };
    boost::generate(data, random_val);
    this->upload_data(data, w);
  }
};

TEMPLATE_PRODUCT_TEST_CASE(
    "[tex1D] upload and download", "[texture1d][texture]", RandomTexture1D,
