...
void main(void) {
  ...
  uint walker_index = imageLoad(head_index, ivec2(gl_FragCoord.xy)).r;

  // Check, if a fragment was written.
  if (walker_index == 0xffffffff)
    ...
    while (walker_index != 0xffffffff && num_frags < MAX_FRAGMENTS) {
      frag_indices[num_frags++] = walker_index;
      walker_index = nodes[walker_index].next_index;
    }
