...
class Texture3D : public BaseTexture<GL_TEXTURE_3D, type, components> {
 ...
 public:
  ...
  //----------------------------------------------------------------------------
  Texture3D(size_t w, size_t h, size_t d,
  ...
  void bind_image_texture_layer(GLuint unit, GLint layer) const {
    gl::bind_image_texture(unit, this->m_id, 0, GL_TRUE, layer, GL_READ_ONLY,
                           gl_internal_format);
  }

  //------------------------------------------------------------------------------
  void bind_image_texture_layer(GLuint unit, GLint layer) {
    gl::bind_image_texture(unit, this->m_id, 0, GL_TRUE, layer, GL_READ_WRITE,
                           gl_internal_format);
  }
 ...
 public:
  //----------------------------------------------------------------------------
  template <typename S>
  void           upload_data(size_t w, size_t h, size_t d,
                             const std::vector<S>& data);
...
//------------------------------------------------------------------------------
template <typename type, typename components>
Texture3D<type, components>::Texture3D(size_t w, size_t h, size_t d, 
                                    tex::InterpolationMode interp_mode_min,
                                    tex::InterpolationMode interp_mode_mag,
                                    tex::WrapMode          wrap_mode_s,
                                    tex::WrapMode          wrap_mode_t,
