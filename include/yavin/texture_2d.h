...
class Texture2D : public BaseTexture<GL_TEXTURE_2D, type, components> {
 public:
  //============================================================================
  using this_t                   = Texture2D<type, components>;
  using parent_t                 = BaseTexture<GL_TEXTURE_2D, type, components>;
  static constexpr bool is_float = parent_t::is_float;
 ...
 public:
  ...
  Texture2D(size_t w, size_t h)
      : Texture2D(w, h, default_interpolation, default_interpolation,
                  default_wrap_mode, default_wrap_mode) {}

  //----------------------------------------------------------------------------
  Texture2D(size_t w, size_t h, tex::InterpolationMode interp_mode,
            tex::WrapMode wrap_mode)
      : Texture2D(w, h, interp_mode, interp_mode, wrap_mode,
                  wrap_mode) {}

  //----------------------------------------------------------------------------
  Texture2D(size_t w, size_t h, tex::InterpolationMode interp_mode_min,
            tex::InterpolationMode interp_mode_mag, tex::WrapMode wrap_mode_s,
            tex::WrapMode wrap_mode_t);

  //----------------------------------------------------------------------------
  template <typename S>
  Texture2D(size_t w, size_t h, const std::vector<S>& data)
      : Texture2D(w, h, data, default_interpolation,
  ...
  Texture2D(size_t w, size_t h, const std::vector<S>& data,
      ...
      : Texture2D(w, h, data, interp_mode, interp_mode, wrap_mode,
                  wrap_mode) {}
  ...
  //----------------------------------------------------------------------------
  auto weight() const { return m_width; }
 ...
 public:
  //----------------------------------------------------------------------------
  template <typename S>
  void upload_data(size_t w, size_t h, const std::vector<S>& data);
...
//------------------------------------------------------------------------------
template <typename type, typename components>
Texture2D<type, components>::Texture2D(size_t w, size_t h,
                                       tex::InterpolationMode interp_mode_min,
                                       tex::InterpolationMode interp_mode_mag,
                                       tex::WrapMode          wrap_mode_s,
                                       tex::WrapMode          wrap_mode_t)
...
void Texture2D<type, components>::unbind_image_texture(unsigned int i) {
  gl::bind_image_texture(i, 0, 0, GL_FALSE, 0, GL_READ_WRITE,
                         gl_internal_format);
}
...
void Texture2D<type, components>::upload_data(const std::vector<type>& data) {
  assert(data.size() == m_width * m_height * num_components);
  this->bind();
...
template <typename type, typename components>
template <typename S>
void Texture2D<type, components>::upload_data(size_t w, size_t h,
                                              const std::vector<S>& data) {
  m_width  = w;
  m_height = h;
...
void Texture2D<type, components>::set_data(const PixelUnpackBuffer<type>& pbo) {
  bind();
  pbo.bind();
  gl::tex_sub_image_2d(GL_TEXTURE_2D, 0, 0, 0, m_width, m_height, gl_format,
                       gl_type, 0);
  pbo.unbind();
}
...
//------------------------------------------------------------------------------
template <typename type, typename components>
void Texture2D<type, components>::resize(size_t w, size_t h) {
...
std::vector<type> Texture2D<type, components>::download_data() const {
  ...
  gl::get_texture_image(this->m_id, 0, gl_format, gl_type,
                        m_width * m_height * num_components * sizeof(type),
                        data.data());
  return data;
}
...
struct tex_png<type, R> {
  ...
  static constexpr auto save_pixel = [](std::vector<type>& data, png_t& image,
    ...
    if constexpr (std::is_same_v<float, type>) {
      ...
      image[image.get_height() - 1 - y][x].red =
          data[idx * num_components] * scale_factor;
      image[image.get_height() - 1 - y][x].green =
          data[idx * num_components] * scale_factor;
      image[image.get_height() - 1 - y][x].blue =
          data[idx * num_components] * scale_factor;
