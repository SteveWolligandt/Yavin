...
inline void  set_viewport(const glm::vec4& vp) {
  set_viewport((unsigned int)vp[0], (unsigned int)vp[1], (unsigned int)vp[2],
               (unsigned int)vp[3]);
}
inline void set_viewport(const Camera& cam) {
  set_viewport((unsigned int)cam.viewport().x, (unsigned int)cam.viewport().y,
               (unsigned int)cam.viewport().z, (unsigned int)cam.viewport().w);
}
}  // namespace Yavin
